...
Firstly, we implemented and tested multiple object-detection architectures, and we compared them against each other on the stage of the dataset that was currently available.
...
After selecting models, we ensembled them using the WBF method. Model weights and IOU threshold $T$ value were optimized on the validation dataset as described in section \cite{sec: ensembling}. The Optuna optimization software was used to find those, where we set search-space to be in the range from 0 to 1 for all model weights as well as for the IOU threshold $T$. The optimization process was terminated after $3000$ trials.
