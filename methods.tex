...
The dimensions to which to resize an image were selected due to limitations of some of the architectures, where they require the input to be divisible by 128. In the beginning, we used square images, due to the ease of implementation. After that, we switched to rectangular images, however no improvement on measured metrics was observed. Based on the architecture, we noticed a decrease in GPU memory usage in the range from 5\% to 10\%.
...
Two YOLOv5 models with large bacbone, Two YOLOv5 with medium-sized backbone, small YOLOv5 model, RetinaNet model with  EfficientDet with D1 and D3 backbones
