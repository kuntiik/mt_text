...
Section \ref{sec:caries_detection} describes the baseline solution. Especially we describe the training protocol used in the remaining parts.

Section \ref{sec:methods:improvements} introduces a couple of changes to training protocol described in section \ref{sec:caries_detection}.
...
Section \ref{sec:methods:dental_restorations} proposes deep learning and non-deep learning approach to dental restorations segmentation.

\section{Caries detection - baseline model comparison}
\label{sec:caries_detection}


Firstly, we implemented and tested multiple object-detection architectures and compared them against each other on the stage of the currently available dataset. The training protocol used for used during training of all models is described by the following:

\subsection{Dataset}
We used the first five stages of the dataset mentioned in chapter \ref{chapter:dataset}. The dataset was always split into train, validation, and test parts, each consisting of 70\%, 15\%, and 15\% of the dataset.
...
The dimensions to which to resize an image were selected due to the limitations of some of the architectures, where they require the input to be divisible by 128. In the beginning, we used square images due to the ease of implementation. After that, we switched to rectangular images, however no improvement in measured metrics was observed. Based on the architecture, we noticed a decrease in GPU memory usage in the range from 5\% to 10\%.k
...
The batch size differed based on the model's architecture and the selected backbone. We always used the biggest batch size to fit into the graphic card memory. An overview of batch sizes for a different combination of architectures and backbones is in table \ref{tab:batch_sizes}. Note that those batch size values allow for the accumulation of four forward passes into the GPU memory.
...
To start with, we experimented with different learning rates and used the learning rate test to find the initial learning rate. There was no difference in the final performance of the model, and the number of epochs required to train the model did not differ significantly. In rare cases, the learning rate test's learning rate did not lead to model convergence. We, therefore, set the initial learning rate to $10^{-4}$ and used a Reduced on Plateau Learning Rate scheduler. The value monitored by the scheduler was the validation loss, and the learning rate decreased by a factor of 5 when the improvement of the loss stalled for five consecutive epochs.
...
In this section, we describe our approach to the model ensembling. First, we describe the process of unifying predictions into the same format. We then describe how optimal hyper-parameters of the ensembling process can be obtained. In the end, we assess the importance of diversity of architectures during the ensembling.
...
We performed grid search over hyper-parameters in the following manner: Defined parameter search space, which can be seen in table \ref{tab:ensembling_search_space}. We evaluated $AP@.5$ on every image of the validation dataset and averaged those values. The best hyper-parameters were selected, and we used them for evaluation on the test dataset. The whole workflow is shown in figure \ref{fig:diag:ense_search}.
\begin{table}
    \centering
    \begin{tabular}{|c|c|c|c|}
        \hline
...
After selecting models, we ensembled them using the WBF method. Model weights and IOU threshold $T$ value were optimized on the validation dataset as described in section \ref{sec: ensembling}. The Optuna optimization software was used to find those, where we set search-space to be in the range from 0 to 1 for all model weights as well as for the IOU threshold $T$. The optimization process was terminated after $3000$ trials.
...
We used YOLOv5 architecture again but included two models with a small backbone, 3 with a medium-sized backbone, and 3 with a large backbone. When picking those, we tried to match the performance of selected models with those selected before. This was done to ensure the maximal comparability of those two ensembles.

\subsection{Different architecture}
Here we used the best model we had at our disposal at the moment. We used the following models: Two YOLOv5-large, two YOLOv5-medium, YOLOv5-small, EfficientDet-D1, EfficientDet-D3, RetinaNet-swint, Faster-RCNN-Resnet50, and Faster-RCNN-Resnet50. Even though we picked the best available models, the difference in $AP@.5$ against YOLOv5-all was $2\$$, as can be seen in table \ref{tab:ensemble_models_involved}, where we denoted a group of models with different architectures as 'All'.

Two YOLOv5 models with large backbone, Three YOLOv5 with medium-sized backbones, small YOLOv5 model, RetinaNet model with  EfficientDet with D1 and D3 backbones
...
In this section, we propose a non-deep learning method for the segmentation of dental caries; we tune hyper-parameters of this approach to achieve the best possible performance of this approach. After that we switch our approach and train a deep learning model called U-Net.
