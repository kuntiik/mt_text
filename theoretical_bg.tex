...
\end{align}
where $\mathcal{M}$ is currently processed bounding box and $\mathcal{D}$ is set of already processed boxes. After processing all boxes, those with $s_i < T$ are removed, where $T$ is confidence cut-off threshold \cite{Bodla2017}.
...
Weighted boxes fusion is combining boxes similarly as NMW. The main difference are: Iterative approach to the fusion, outputing confidence for the merged box and awarnes of number of models, which contributed to the prediction.
